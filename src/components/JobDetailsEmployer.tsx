...
export default function JobDetailsEmployer() {
  ...
  const [expandedApplicant, setExpandedApplicant] = useState(null); // State to track expanded applicant
  ...
  useEffect(() => {
    const fetchJobAndApplicants = async () => {
      ...
      try {
        ...
        setJob(data.results.job); // Set the job details
        setApplicants(data.results.applicants); // Set the applicants
      } catch (err) {
        console.error(err);
      } finally {
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <Badge variant="outline" className="flex items-center gap-1 border-none p-0">
                ...
                job.type.map((type, index) => (
                  <Badge key={index} variant="secondary" className={"bg-transparent py-0 px-0"}>
                    {type === "full-time" ? (
                      ...
                      <span className="bg-red-100 text-red-700 px-3 py-[2px] rounded-sm">Contract</span>
                    ) : type === "internship" ? (
                      <span className="bg-yellow-100 text-yellow-900 px-3 py-[2px] rounded-sm">Internship</span>
                    ) : (
        ...
        {filteredApplicants.length === 0 ? (
          ...
          filteredApplicants.map((applicant) => (
            <ApplicantCard
              ...
              onUpdateStatus={(status) => updateApplicationStatus(applicant.id, status)}
            />
          ))
        )}
      </div>
